...
titre_csre_4Mb <- variables[1]
titre_landraces_4Mb <- variables[2]
titre_FST_SNP <- variables[3]
titre_FST_blocs <- variables[4]
titre_rho_aleatoires<- variables[5]
titre_lambda_aleatoires<- variables[6]
titre_hrec<- variables[7]
titre_csre <- variables[8]
titre_domestication <- variables[9]
titre_cliques_HR <- variables[10]
titre_chr_tab <- variables[11]
titre_matrice_fst_blocs <- variables[12]
titre_graphe_chromosomes <- variables[13]
titre_graphe_domestication <- variables[14]
titre_graphe_HR_partages <- variables[15]
titre_f2 <- variables[16]
titre_f3 <- variables[17]
titre_f4 <- variables[18]
titre_f6 <- variables[19]
titre_graph_introgressions <- variables[20]
titre_example_correlation_lambda_fst_3BR1 <- variables[21]
titre_correlation_lambda_FST_SNP <- variables[22]
titre_correlation_rho_FST_haplotypic_blocks <- variables[23]
...
csre %>% filter(region=="C") %>% group_by(region) %>% summarise(null_intervals_c=round(length(which(cemp==0))/nrow(.),2))

carte_csre_landraces_4Mb %>% filter(chr=="7D" & region=="R3") %>% group_by(population) %>% summarise(c=cor(lambda_rho, cbay))
carte_csre_landraces_4Mb %>% filter(chr=="2A" & region=="R1") %>% group_by(population) %>% summarise(c=cor(lambda_rho, cbay))
...
head(fread(titre_FST_blocs))
head(fread(titre_rho_aleatoires))
head(fread(titre_lambda_aleatoires))
head(fread(titre_hrec))
head(fread(titre_csre))
head(fread(titre_domestication))
head(fread(titre_cliques_HR))
head(fread(titre_chr_tab))
fread(titre_matrice_fst_blocs)
...
csre_4Mb <- fread(titre_csre_4Mb) 

landraces_4Mb <- fread(titre_landraces_4Mb) 

csre <- fread(titre_csre)

lal <- fread(titre_lambda_aleatoires)

matrice_fst <- read.table(titre_matrice_fst_blocs, header = T, row.names = 1)
...
# for comparison of rho and CsRe variations
carte_csre_landraces_4Mb <- csre_4Mb %>% 
  dplyr::select(chr, posl,posr,cbay) %>% 
  inner_join(landraces_4Mb, by=c("chr","posl","posr")) %>%
  dplyr::select(population, chr, region, posl, posr, cbay, lambda_rho) %>%
...
#### correlation per genomic region
carte_csre_landraces_4Mb %>% group_by(population, chr, region) %>%
  summarise(c=cor(lambda_rho, cbay)) %>%
  mutate(region2=ifelse(region %in% c("R1","R2a","R2b","R3"), "telo", "centro")) %>%
  group_by(population, region2) %>%
  summarise(sd=round(sd(c), digits=2), c=round(mean(c), 2)) 
...
# for relationship between historical profiles and Fst
col <- viridis(7)
...
# Figure 1 : Lines for recombination profiles on 3B chromosome

# csre
f2a <- carte_csre_landraces_4Mb %>% filter(chr=="3B") %>%
  rbind(data=carte_csre_landraces_4Mb %>% filter(chr=="3B") %>%
...
# landraces
f2b <- carte_csre_landraces_4Mb %>% filter(chr=="3B") %>%
  rbind(data=carte_csre_landraces_4Mb %>% filter(chr=="3B") %>%
          group_by(population, region) %>%
          arrange(posl) %>%
          slice(n()) %>% 
          ...
          na.omit() %>%
          filter(region !="R1")) %>%
  ggplot(aes(x=posl, y=lambda_rho, col=region)) + geom_line() +
...
# Figure 3 : Scatter plot for relationship rho and CsRe at 4Mb per region R1...R3


# all regions combined
# f3a <- carte_csre_landraces_4Mb %>% filter(population=="WE") %>%
#   ggplot(aes(x=cbay, y=lambda_rho, col=region)) +
#   geom_point()+
...
#   theme(plot.title = element_text(hjust = 0.5, size=police_gros_titres),
#         strip.text.y = element_text(size = police_axes, color="black"),
#         axis.text.y = element_text(size=police_axes),
#         axis.title.y = element_text(size=police_titres),
#         axis.text.x = element_text(size=police_axes),
#         axis.title.x = element_text(size=police_titres),
#         legend.text=element_text(size=police_axes),
#         legend.title=element_text(size=police_titres),
#         strip.background = element_rect(color="black", fill="white"))+ 
#   scale_colour_manual(values=regions)+
...
f3a <- carte_csre_landraces_4Mb %>% filter(population=="WE") %>%
  ...
  geom_point(size=0.5)+
  theme_light() +
  scale_x_continuous(trans="log10", breaks = c(1e-2, 1e-1,1, 10), limits=c(min(carte_csre_landraces_4Mb$cbay), max(carte_csre_landraces_4Mb$cbay)), labels = function(x) ifelse(x == 1, "1", x)) +
  scale_y_continuous(trans="log10", breaks = c(1e-5,1e-3,1e-1), limits = c(min(carte_csre_landraces_4Mb$lambda_rho), 1e-1))  +
...
# R1
f3b <- carte_csre_landraces_4Mb %>% filter(population=="WE") %>%
  ggplot(aes(x=cbay, y=lambda_rho, col=region)) +
  scale_colour_manual(values=regions)+
  geom_point(data=carte_csre_landraces_4Mb %>%
               filter(population=="WE") , aes(x=cbay, y=lambda_rho), col="white", alpha=1, size=0.5) +
  geom_point(data=carte_csre_landraces_4Mb %>%
               filter(population=="WE") , aes(x=cbay, y=lambda_rho), col="lightgrey", alpha=1, size=0.5)+
  theme_light() +
  xlab("") +
  ylab("")  + 
  geom_point(data=carte_csre_landraces_4Mb %>%
               filter(population=="WE" & region=="R1") , aes(x=cbay, y=lambda_rho), col=R1, size=0.5)  +
  scale_x_continuous(trans="log10", breaks = c(1e-2, 1e-1,1, 10), limits=c(min(carte_csre_landraces_4Mb$cbay), max(carte_csre_landraces_4Mb$cbay)), labels = function(x) ifelse(x == 1, "1", x)) +
  scale_y_continuous(trans="log10", breaks = c(1e-5,1e-3,1e-1), limits = c(min(carte_csre_landraces_4Mb$lambda_rho), 1e-1))  +
  theme(plot.title = element_text(hjust = 0.5, size=14, vjust=-1),
        axis.text.y =element_text(size=14),
...
# R2a
# f3c<- carte_csre_landraces_4Mb %>% filter(population=="WE") %>%
#   ggplot(aes(x=cbay, y=lambda_rho, col=region)) +
#   scale_colour_manual(values=regions)+
#   geom_point(data=carte_csre_landraces_4Mb %>%
#                filter(population=="WE") , aes(x=cbay, y=lambda_rho), col="white", alpha=1) +
#   geom_point(data=carte_csre_landraces_4Mb %>%
#                filter(population=="WE") , aes(x=cbay, y=lambda_rho), col="lightgrey", alpha=1)+
#   scale_y_continuous(trans="log10") +
#   scale_x_continuous(trans="log10") +
#   theme_light() +
#   xlab("") +
#   ylab(expression(paste("WE ", rho, " (/kb)")))  + 
#   geom_point(data=carte_csre_landraces_4Mb %>%
#                filter(population=="WE" & region=="R2a") , aes(x=cbay, y=lambda_rho), col=R2a) +
#   theme(plot.title = element_text(hjust = 0.5, size=police_gros_titres),
#         strip.text.y = element_text(size = police_axes, color="black"),
#         axis.text.y = element_text(size=police_axes),
#         axis.title.y = element_text(size=police_titres),
...
f3c<- carte_csre_landraces_4Mb %>% filter(population=="WE") %>%
  ...
  ylab("")  + 
  geom_point(data=carte_csre_landraces_4Mb %>%
               filter(population=="WE" & region=="R2a") , aes(x=cbay, y=lambda_rho), col=R2a, size=0.5)  +
  scale_x_continuous(trans="log10", breaks = c(1e-2, 1e-1,1, 10), limits=c(min(carte_csre_landraces_4Mb$cbay), max(carte_csre_landraces_4Mb$cbay)), labels = function(x) ifelse(x == 1, "1", x)) +
  scale_y_continuous(trans="log10", breaks = c(1e-5,1e-3,1e-1), limits = c(min(carte_csre_landraces_4Mb$lambda_rho), 1e-1))  +
  theme(plot.title = element_text(hjust = 0.5, size=14, vjust=-1),
        axis.text.y =element_text(size=14),
  ...
  ggtitle(("R2a"))
# C
# f3d<-carte_csre_landraces_4Mb %>% filter(population=="WE") %>%
#   ggplot(aes(x=cbay, y=lambda_rho, col=region)) +
#   scale_colour_manual(values=regions)+
#   geom_point(data=carte_csre_landraces_4Mb %>%
#                filter(population=="WE") , aes(x=cbay, y=lambda_rho), col="white", alpha=1) +
...
#   ylab("")  + 
#   geom_point(data=carte_csre_landraces_4Mb %>%
#                filter(population=="WE" & region=="C") , aes(x=cbay, y=lambda_rho), col=C) +
#   theme(plot.title = element_text(hjust = 0.5, size=police_gros_titres),
#         strip.text.y = element_text(size = police_axes, color="black"),
#         axis.text.y = element_text(size=police_axes),
#         axis.title.y = element_text(size=police_titres),
...
f3e<- carte_csre_landraces_4Mb %>% filter(population=="WE") %>%
  ggplot(aes(x=cbay, y=lambda_rho, col=region)) +
  scale_colour_manual(values=regions)+
  geom_point(data=carte_csre_landraces_4Mb %>%
               filter(population=="WE") , aes(x=cbay, y=lambda_rho), col="white", alpha=1, size=0.5) +
  ...
  ylab("")  + 
  geom_point(data=carte_csre_landraces_4Mb %>%
               filter(population=="WE" & region=="R2b") , aes(x=cbay, y=lambda_rho), col=R2b, size=0.5)  +
  scale_x_continuous(trans="log10", breaks = c(1e-2, 1e-1,1, 10), limits=c(min(carte_csre_landraces_4Mb$cbay), max(carte_csre_landraces_4Mb$cbay)), labels = function(x) ifelse(x == 1, "1", x)) +
  scale_y_continuous(trans="log10", breaks = c(1e-5,1e-3,1e-1), limits = c(min(carte_csre_landraces_4Mb$lambda_rho), 1e-1)) +
  theme(plot.title = element_text(hjust = 0.5, size=14, vjust=-1),
        axis.text.y =element_text(size=14),
 ...
 f3d<- carte_csre_landraces_4Mb %>% filter(population=="WE") %>%
   ...
   ylab("")  + 
   geom_point(data=carte_csre_landraces_4Mb %>%
                filter(population=="WE" & region=="C") , aes(x=cbay, y=lambda_rho), col=C, size=0.5)  +
   scale_x_continuous(trans="log10", breaks = c(1e-2, 1e-1,1, 10), limits=c(min(carte_csre_landraces_4Mb$cbay), max(carte_csre_landraces_4Mb$cbay)), labels = function(x) ifelse(x == 1, "1", x)) +
   scale_y_continuous(trans="log10", breaks = c(1e-5,1e-3,1e-1), limits = c(min(carte_csre_landraces_4Mb$lambda_rho), 1e-1))+
   theme(plot.title = element_text(hjust = 0.5, size=14, vjust=-1),
         axis.text.y =element_text(size=14),
...
#   scale_x_continuous(trans="log10") +
#   theme_light() +
#   xlab("CsRe Bayesian rec rates\n (cM/Mb)") +
#   ylab(expression(paste("WE ", rho, " (/kb)")))  + 
#   geom_point(data=carte_csre_landraces_4Mb %>%
#                filter(population=="WE" & region=="R2b") , aes(x=cbay, y=lambda_rho), col=R2b) +
#   theme(plot.title = element_text(hjust = 0.5, size=police_gros_titres),
#         strip.text.y = element_text(size = police_axes, color="black"),
#         axis.text.y = element_text(size=police_axes),
#         axis.title.y = element_text(size=police_titres),
...
# R3
# f3f<-  carte_csre_landraces_4Mb %>% filter(population=="WE") %>%
#   ggplot(aes(x=cbay, y=lambda_rho, col=region)) +
#   scale_colour_manual(values=regions)+
#   geom_point(data=carte_csre_landraces_4Mb %>%
#                filter(population=="WE") , aes(x=cbay, y=lambda_rho), col="white", alpha=1) +
...
#   ylab("")  + 
#   geom_point(data=carte_csre_landraces_4Mb %>%
#                filter(population=="WE" & region=="R3") , aes(x=cbay, y=lambda_rho), col=R3) +
#   theme(plot.title = element_text(hjust = 0.5, size=police_gros_titres),
#         strip.text.y = element_text(size = police_axes, color="black"),
#         axis.text.y = element_text(size=police_axes),
#         axis.title.y = element_text(size=police_titres),
...
f3f<- carte_csre_landraces_4Mb %>% filter(population=="WE") %>%
  ...
  ylab("")  + 
  geom_point(data=carte_csre_landraces_4Mb %>%
               filter(population=="WE" & region=="R3") , aes(x=cbay, y=lambda_rho), col=R3, size=0.5)  +
  scale_x_continuous(trans="log10", breaks = c(1e-2, 1e-1,1, 10), limits=c(min(carte_csre_landraces_4Mb$cbay), max(carte_csre_landraces_4Mb$cbay)), labels = function(x) ifelse(x == 1, "1", x)) +
  scale_y_continuous(trans="log10", breaks = c(1e-5,1e-3,1e-1), limits = c(min(carte_csre_landraces_4Mb$lambda_rho), 1e-1)) +
  theme(plot.title = element_text(hjust = 0.5, size=14, vjust=-1),
        axis.text.y =element_text(size=14),
...
# 
# f3 <- ggarrange(f3a, f3b, f3c, f3d, f3e, f3f, nrow=3, ncol=2, common.legend = T, legend = "none")
# f3
# tiff(titre_f3, units="in", width = 12, height = 18, res=200, compression = "lzw")
# f3
# dev.off()
# 
# 
# 
# # Figure 4 : boxplots for correlation of rho and CsRe at 4Mb per region R1...R3
# temp <- carte_csre_landraces_4Mb %>%
#   group_by(population, chr, region) %>%
#   summarise(correlation=cor(lambda_rho, cbay)) %>%
#   mutate(intogressions=case_when(chr=="7D" & region=="R3"~"7DR3",
#                                  chr=="2A" & region=="R1"~"2AR1")) %>%
#   mutate(region=factor(region, levels=c("R1","R2a","C","R2b","R3"))) %>%
#   na.omit()
#   
# 
# f4 <- carte_csre_landraces_4Mb %>% group_by(population, chr, region ) %>%
#   summarise(correlation=cor(lambda_rho, cbay), nb_intervalles=n())  %>%
#   filter(nb_intervalles>=5) %>%
#   arrange(correlation) %>% 
#   mutate(region=factor(region, levels=c("R1","R2a","C","R2b","R3"))) %>%
#   na.omit() %>%
#   mutate(introgressions=case_when(chr=="7D" & region=="R3"~"7DR3")) %>%
#   rowwise() %>%
#   mutate(introgressions=sample(c("A","B","C"), size=1, replace=T)) %>%
...
#         legend.title=element_text(size=16),legend.key = element_rect(colour = NA, fill = NA),
#         strip.background = element_rect(color="black", fill="white"))+ 
#   guides(color=guide_legend(override.aes=list(fill=NA)))+
#   theme(legend.key = element_rect(fill = NA, color = NA))+
#   scale_colour_manual(values=regions) +
#   geom_point(aes(x=1+0.3,y=-0.328, shape="7DR3", col="black"), size=3, col="black")+
#   geom_point(aes(x=2+0.3,y=-0.192, shape="7DR3", col="black"), size=3, col="black")+
#   geom_point(aes(x=3+0.3,y=-0.370, shape="7DR3", col="black"), size=3, col="black")+
#   geom_point(aes(x=4+0.3,y=-0.289, shape="7DR3", col="black"), size=3, col="black")+
#   geom_point(aes(x=1-0.3,y=0.612, shape="2AR1", col="black"), size=3, col="black")+
#   geom_point(aes(x=2-0.3,y=0.693, shape="2AR1", col="black"), size=3, col="black")+
#   geom_point(aes(x=3-0.3,y=0.692, shape="2AR1", col="black"), size=3, col="black")+
#   geom_point(aes(x=4-0.3,y=0.632, shape="2AR1", col="black"), size=3, col="black")+
#   
#   scale_shape_manual(name = "introgressions",
#                      breaks = c("none", "2AR1","7DR3"),
#                      values = c(8,4, 20) )+ theme(legend.key=element_blank())+
...
f3 <-ggdraw() +
  draw_plot(f3e, x=0+0.02,y=0.12, width=0.2, height = 0.2, scale=1.7) +
  draw_plot(f3c, x=0+0.02,y=0.45, width=0.2, height = 0.2, scale=1.5) +
  draw_plot(f3a, x=0+0.02,y=0.75, width=0.2, height = 0.2, scale=1.5) +
  draw_plot(f3f, x=0.15+0.06,y=0.12, width=0.2, height = 0.2, scale=1.7) +
  draw_plot(f3d, x=0.15+0.06,y=0.45, width=0.2, height = 0.2, scale=1.5) +
  draw_plot(f3b, x=0.15+0.06,y=0.75, width=0.2, height = 0.2, scale=1.5) +
  draw_plot(f4, x = 0.45, width=0.56) +
...
lal %>% group_by(p) %>%
  summarise(c=round(mean(correlation), digits=2), sd=round(sd(correlation), digits=2)) %>%
  arrange(desc(c))
t <- pairwise.t.test(lal$correlation, lal$p, p.adj = "bonf")$p.value
...
f4 <- ggplot(matrice_fst3, aes(x=p,y=correlation, fill=p)) + geom_boxplot()+
  scale_fill_manual(values=c(viridis(t)[n+6],viridis(t)[n+5],viridis(t)[n+4],viridis(t)[n+3],viridis(t)[n+2],viridis(t)[n+1]), 
                    labels=c(round(unique(matrice_fst3$FST)*100, digits=1)), name="")+
  ...
  guides(fill = guide_legend(              label.hjust = 0.3,              label.position = "top",
                             title.position = "top", title.hjust = 0.5,
                             nrow = 1,override.aes=list(fill=NA, col=NA, shape=NA))) +

  xlab("") +
  stat_summary(geom = 'text', label = texte$signif, fun.y = max, vjust = -1, size=10)  +
  scale_y_continuous(limits = c(min(lal2$correlation), max(lal2$correlation)+0.1))+
...
# Figure 7 : relation correlation lambda dn FST per genomic regin 1AR1...7DR3



f6a <- cor_lambda_fst_blocs %>%
...
f6b <- eff.FST.cor_lambda_fst_blocs %>%
